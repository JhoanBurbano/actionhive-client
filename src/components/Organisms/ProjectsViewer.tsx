...
const ProjectsViewer: React.FC<ProjectsViewerProps> = ({
  ...
  return (
    <>
          ...
          >
            {projects.map(({id, projectName, description, developmentStatus, fundingCap, representant}, i) => (
              <SwiperSlide key={i}>
                <article className="project-viewer__item" onClick={()=>goToDetails(id)}>
                  <img
                  ...
                  <span className="project-viewer__item-body-objetivo">
                    ...
                    <p>COP {fundingCap!.toString().replace(CURRENCY_REGEX, '.')}</p>
                  </span>
                  </section>
                </article>
              </SwiperSlide>
